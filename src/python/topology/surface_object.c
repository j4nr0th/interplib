...
static PyObject *surface_object_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    {
        if (geo_id_from_object(state->geoid_type, PySequence_Fast_GET_ITEM(seq, i), this->lines + i) < 0)
        {
            Py_DECREF(this);
            Py_DECREF(seq);
            return NULL;
...
{
    return (surface_object_t *)surf_type->tp_alloc(surf_type, (Py_ssize_t)count);
}

INTERPLIB_INTERNAL
surface_object_t *surface_object_from_value(PyTypeObject *surf_type, size_t count, geo_id_t ids[static count])
{
    surface_object_t *const this = surface_object_empty(surf_type, count);
    if (!this)
    {
        return NULL;
    }
...
static PyObject *surface_sequence_item(PyObject *self, Py_ssize_t idx)
{
    ...
    PyObject *const mod = PyType_GetModule(Py_TYPE(this));
    if (!mod)
    {
        return NULL;
    }
    ...
    return (PyObject *)geo_id_object_from_value(state->geoid_type, this->lines[idx]);
}

static PyType_Slot surface_type_slots[] = {
    {Py_tp_repr, surface_object_repr},
    {Py_tp_str, surface_object_str},
    {Py_tp_doc, (void *)surface_object_type_docstring},
    {Py_tp_richcompare, surface_object_rich_compare},
    {Py_tp_new, surface_object_new},
    {Py_tp_methods, surface_methods},
    {Py_sq_length, surface_sequence_length},
    {Py_sq_item, surface_sequence_item},
    {0, NULL},
};

INTERPLIB_INTERNAL
PyType_Spec surface_type_spec = {
    .name = "interplib._interp.Surface",
    .basicsize = sizeof(surface_object_t),
    .itemsize = sizeof(geo_id_t),
    .flags = Py_TPFLAGS_DEFAULT,
    .slots = surface_type_slots,
};
