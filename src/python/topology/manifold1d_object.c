...
static PyObject *manifold1d_get_n_lines(PyObject *self, void *Py_UNUSED(closure))
{
    ...
    return PyLong_FromSize_t((size_t)this->manifold.n_lines);
}

static PyObject *manifold1d_get_n_points(PyObject *self, void *Py_UNUSED(closure))
{
    const manifold1d_object_t *this = (manifold1d_object_t *)self;
    return PyLong_FromSize_t((size_t)this->manifold.n_points);
}
...
{
    const interplib_module_state_t *const state = (interplib_module_state_t *)PyType_GetModuleState(Py_TYPE(self));
    if (!state)
    {
        return NULL;
    }
    ...
    {
        PyErr_Format(PyExc_IndexError, "The manifold has %u lines, but index %u was specified.",
                     (unsigned)this->manifold.n_lines, (unsigned)id.index);
        return NULL;
    }
    const line_t *p_line = this->manifold.lines + id.index;
    if (id.reverse)
    {
        return (PyObject *)line_from_indices(state->line_type, p_line->end, p_line->begin);
    }
    return (PyObject *)line_from_indices(state->line_type, p_line->begin, p_line->end);
}
...
static PyObject *manifold1d_find_line(PyObject *self, PyObject *arg)
{
    ...
    const manifold1d_object_t *const this = (manifold1d_object_t *)self;
    if (!PyObject_TypeCheck(arg, state->line_type))
    {
    ...
    if (line.begin.index >= this->manifold.n_points || line.end.index >= this->manifold.n_points)
    {
        return (PyObject *)geo_id_object_from_value(state->geoid_type,
                                                    (geo_id_t){.index = GEO_ID_INVALID, .reverse = 0});
    }

    for (size_t line_id = 0; line_id < this->manifold.n_lines; ++line_id)
    {
        const line_t *p_line = this->manifold.lines + line_id;
        if (geo_id_compare(p_line->begin, line.begin) && geo_id_compare(p_line->end, line.end))
        {
            return (PyObject *)geo_id_object_from_value(state->geoid_type, (geo_id_t){.index = line_id, .reverse = 0});
        }
        if (geo_id_compare(p_line->begin, line.end) && geo_id_compare(p_line->end, line.begin))
        {
            return (PyObject *)geo_id_object_from_value(state->geoid_type, (geo_id_t){.index = line_id, .reverse = 1});
        }
    }
    return (PyObject *)geo_id_object_from_value(state->geoid_type, (geo_id_t){.index = GEO_ID_INVALID, .reverse = 0});
}
...
static PyObject *manifold1d_line_mesh(PyObject *cls, PyObject *arg)
{
    ...
    if (!this)
        return NULL;
    const interp_result_t res = manifold1d_new_line(&this->manifold, n + 1, &SYSTEM_ALLOCATOR);
    if (res != INTERP_SUCCESS)
    {
        PyErr_Format(PyExc_RuntimeError, "Failed to create manifold: %s (%s).", interp_error_str(res),
                     interp_error_msg(res));
        Py_DECREF(this);
        return NULL;
    }
    // this->manifold.n_points = n + 1;
    // this->manifold.n_lines = n;
    // for (size_t i = 0; i < n; ++i)
    // {
    //     this->manifold.lines[i] = (line_t){.begin = {.index = i, .reverse = 0}, .end = {.index = i + 1, .reverse =
    //     0}};
    // }
...
{
    const manifold1d_object_t *this = (manifold1d_object_t *)self;
    PyTypeObject *const type = (PyTypeObject *)Py_TYPE(self);

    manifold1d_object_t *const dual = (manifold1d_object_t *)type->tp_alloc(type, 0);
    if (!dual)
        return NULL;

    const interp_result_t res = manifold1d_dual(&this->manifold, &dual->manifold, &SYSTEM_ALLOCATOR);
    if (res != INTERP_SUCCESS)
    {
        PyErr_Format(PyExc_RuntimeError, "Failed to create manifold: %s (%s).", interp_error_str(res),
                     interp_error_msg(res));
...
static void maintainer_dealloc(PyObject *self)
{
    manifold1d_object_t *const this = (manifold1d_object_t *)self;
    manifold1d_free(&this->manifold, &SYSTEM_ALLOCATOR);
...
static PyType_Slot manifold1d_slots[] = {
    {Py_tp_methods, manifold1d_methods},
    {Py_tp_getset, manifold1d_get_set},
    {Py_tp_dealloc, maintainer_dealloc},
    {0, NULL},
};

INTERPLIB_INTERNAL
PyType_Spec manifold1d_type_spec = {
    .name = "interplib._interp.Manifold1D",
    .basicsize = sizeof(manifold1d_object_t),
    .itemsize = sizeof(line_t),
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE | Py_TPFLAGS_HEAPTYPE | Py_TPFLAGS_DISALLOW_INSTANTIATION,
    .slots = manifold1d_slots,
};
