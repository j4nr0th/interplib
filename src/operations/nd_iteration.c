...
static inline size_t *nd_iterator_dims(nd_iterator_t *this)
{
    return this->dims_and_offsets;
}

static inline size_t *nd_iterator_offsets(nd_iterator_t *this)
{
    return this->dims_and_offsets + this->ndims;
}

static inline const size_t *nd_iterator_dims_const(const nd_iterator_t *this)
{
    return this->dims_and_offsets;
}

static inline const size_t *nd_iterator_offsets_const(const nd_iterator_t *this)
{
    return this->dims_and_offsets + this->ndims;
}
...
{
    this->ndims = ndims;
    size_t *const iter_dims = nd_iterator_dims(this);
    size_t *const iter_offsets = nd_iterator_offsets(this);
...
static void nd_iterator_set_to_end(nd_iterator_t *this)
{
    size_t *const iter_offsets = nd_iterator_offsets(this);
    const size_t *const iter_dims = nd_iterator_dims_const(this);
...
compute_offset:;
    ...
    if (dim == 0)
    {
        nd_iterator_restart(this);
        return;
    }

    const size_t remaining = iter_offsets[dim] - step;
    // Ceil-divide
    const size_t required_steps = (remaining + iter_dims[dim] - 1) / iter_dims[dim];

    iter_offsets[dim] = iter_offsets[dim] + required_steps * iter_dims[dim] - remaining;
    // Tail recursion (but with goto)
...
{
    const size_t *const iter_offsets = nd_iterator_offsets_const(this);
    const size_t *const iter_dims = nd_iterator_dims_const(this);
